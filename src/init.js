...
export async function init(setupScene = () => {}, onFrame = () => {}) {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    raySpace.addEventListener('selectstart', () => {
      ...
      if (intersects.length > 0) {
        ...
        console.log('Interacted with:', selectedObject.name);
      }
    });
  ...
  function animate() {
    ...
    Object.values(controllers).forEach((controller) => {
      if (controller?.gamepad) {
        ...
        // Update raycaster position for the controller
        if (controller.raySpace) {
          tempMatrix.identity().extractRotation(controller.raySpace.matrixWorld);
